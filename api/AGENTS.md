...
- It wants to DROP the hand-written functional and partial indexes — `uq_companies_gst_lower`,
  `uq_users_email_lower`, the `lower(name)` ones on the product master and vendors,
  `uq_tickets_slot_token`, the live-invite index. SQLAlchemy cannot express `lower(x)` or a
  `WHERE` clause in an `Index()`, so Alembic cannot see them and mistakes them for stale.
  **Delete those drops.** They are created by `op.execute` at the bottom of the initial migration.
