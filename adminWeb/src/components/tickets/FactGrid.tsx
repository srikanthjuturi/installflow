...
export function FactGrid({ ticket }: { ticket: TicketDetail }) {
  ...
  return (
    // A CONTAINER query, not a viewport one. This grid sits in the narrower
    // half of a two-column ticket layout — `sm:grid-cols-4` looked at the
    // browser window, which is wide on any desktop, and forced 4 columns
    // into a card with nowhere near the room for them, wrapping every label
    // and value mid-word. `@xl` asks about THIS element's own box instead.
    <div className="@container mt-4.5">
      ...
      <dl className="grid grid-cols-2 gap-px overflow-hidden rounded-md border border-line-2 bg-line-2 @xl:grid-cols-4">
        {facts.map(([k, v]) => (
