...
const STATIC: Record<string, Meta> = {
  ...
  "/tickets/new": { title: "Manual ticket entry", subtitle: "Single ticket · ops entry" },
  "/tickets/import": { title: "Excel bulk upload", subtitle: "Template-based import" },
  "/escalations": { title: "Escalation queue", subtitle: "Unassigned within 4h of slot" },
  ...
  "/territory": { title: "Territory mapping", subtitle: "Region → RSH → ASM → pincode" },
...
const DYNAMIC: Array<[RegExp, Meta]> = [
  [/^\/tickets\/import\/[^/]+$/, { title: "Upload validation", subtitle: "Row-level result" }],
  [/^\/tickets\/[^/]+\/force-close$/, { title: "Force closure", subtitle: "Justification required" }],
  [/^\/tickets\/[^/]+$/, { title: "Ticket detail", subtitle: "Timeline & audit trail" }],
  [/^\/escalations\/[^/]+\/bonus$/, { title: "Bonus setup", subtitle: "Re-notification incentive" }],
  [/^\/escalations\/[^/]+\/assign$/, { title: "Manual assignment", subtitle: "Assign a technician" }],
  [/^\/ai-review\/[^/]+$/, { title: "AI review", subtitle: "Image & serial comparison" }],
  [/^\/technicians\/[^/]+$/, { title: "Technician profile", subtitle: "Category · pincode · bandwidth" }],
];
