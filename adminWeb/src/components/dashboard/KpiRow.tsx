      ...
      {kpis.map((k) => {
        const tile = (
          <Card
            className={cn(
              "h-full",
              // Only a tile that leads somewhere reacts to the pointer. The
              // same tint the attention rows use, so the two halves of the
              // screen offer one affordance rather than two.
              k.to && "transition-colors hover:bg-surface-2"
            )}
          >
        ...
        // A real link, not a click handler on the card: it has to be reachable
        // by keyboard, openable in a new tab, and copyable — a manager sending
        // "the breaching list" to somebody is the whole point of a URL that
        // carries the filters.
        return k.to ? (
          <Link
            key={k.key}
