    ...
    <Card className="h-fit [--card-spacing:--spacing(5.5)]">
      <CardContent>
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-ink-3 font-mono text-xs">{tech.id}</p>
          <div className="mt-2">
        ...
        <dl className="border-line-2 divide-line-2 mt-4.5 divide-y overflow-hidden rounded-md border">
          {facts.map(([label, value]) => (
            <div key={label} className="flex items-center justify-between px-3.5 py-2.75">
        ...
        <section className="mt-4">
          <h3 className="text-ink-3 mb-2 text-[11px] font-bold tracking-[0.04em] uppercase">
            Categories
          </h3>
          <ul className="flex flex-wrap gap-1.5">
            {tech.cats.map((c) => (
              <li
                key={c}
                className="bg-surface-3 text-ink-2 rounded-full px-2.5 py-1 text-xs font-medium"
              >
                {c}
              </li>
            ))}
        ...
        <section className="mt-4">
          ...
          <p className="text-ink-2 font-mono text-xs">{tech.pincodes}</p>
        </section>
      </CardContent>
    </Card>
  );
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      ...
      <StatTile label="Net ledger">
        <span className={net < 0 ? "text-danger" : "text-ok"}>{money(net)}</span>
      </StatTile>
    </div>
  );
}
  ...
  return (
    <Card>
      <CardContent>
        ...
        <div className="mt-1 text-[22px] leading-tight font-semibold">{children}</div>
      </CardContent>
    </Card>
  );
}
