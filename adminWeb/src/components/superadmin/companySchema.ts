 ...
 *
 * GSTIN / PAN / PIN / phone formats are validated client-side for fast feedback;
 * the backend re-validates and is the authority. Patterns are case-insensitive —
...
/**
 * 10 digits, optionally with a +91 or 0 in front — the same pattern the vendor
 * form uses, because it is the same kind of value and the API normalises both
 * through `app.core.phone`. Spaces are squashed first, so a number typed as
 * "98200 11001" is the number the user meant rather than a rejection.
 */
...
/**
 * Every field is a string (empty = "not provided" for `adminName`, the only
 * optional one left), so the inferred type matches the all-string form values
 * RHF holds.
 *
