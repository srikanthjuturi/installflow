...
function RegionCard({ region }: { region: RegionTerritory }) {
  return (
    <Card className="[--card-spacing:0rem]">
      <div className="bg-surface-2 border-line-2 flex flex-wrap items-center gap-3 border-b px-4.5 py-3.5">
        <span
          className="bg-brand-500 grid size-9 shrink-0 place-items-center rounded-md text-white"
          aria-hidden
        >
        ...
        <div>
          ...
          <p className="text-ink-3 text-xs">RSH · {region.rsh}</p>
        </div>
...
function AsmRow({ asm }: { asm: AsmTerritory }) {
  return (
    <div className="hover:bg-surface-2 flex flex-wrap items-center gap-3 rounded-md px-3 py-2.75 transition-colors">
      <span
        className="bg-status-assigned-bg text-brand-400 grid size-7.5 shrink-0 place-items-center rounded-full text-[11px] font-semibold"
        aria-hidden
      >
        {asm.initial}
      </span>
      <div className="min-w-37.5">
        ...
        <p className="text-ink-3 text-[11px]">ASM · {asm.area}</p>
      </div>
      <ul className="flex flex-1 flex-wrap gap-1.5" aria-label={`Pincodes serviced by ${asm.name}`}>
        {asm.pincodes.map((pincode) => (
          <li
            key={pincode}
            className="bg-surface-3 text-ink-2 rounded-sm px-2 py-0.75 font-mono text-[11px] font-medium"
          >
            {pincode}
          </li>
        ))}
    ...
    <div className="flex flex-col gap-3">
      {Array.from({ length: regions }).map((_, r) => (
        <Card key={r} className="[--card-spacing:0rem]">
          <div className="bg-surface-2 border-line-2 flex items-center gap-3 border-b px-4.5 py-3.5">
          ...
          <div className="flex flex-col p-2.5">
            {Array.from({ length: asms }).map((__, a) => (
              <div key={a} className="flex flex-wrap items-center gap-3 px-3 py-2.75">
